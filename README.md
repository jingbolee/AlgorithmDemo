...
- 翻转链表
    - 思路：通过栈实现翻转
    - 思路：通过Arraylist，然后倒叙输出
- 两个栈实现一个队列
    - 思路：栈的特点是先进后出，队列的特点是先进先出，根据栈和队列的特点来实现该数据结构问题
    - 栈A 和栈B，栈A作为队列的入队用的，栈B作为队列出队用的。有元素入队，进入到了栈A，然后根据元素的先进后出原则，假如是入队完成了以后再出队，可以把栈A的元素全部压入栈B，这样从栈B出来的元素，就符合队列的特点了。另外一种情况，一遍入队列，一边出队列，入队列进入栈A，出队列从栈B出，假如栈B为空了，就从栈A把所有的元素都放入栈B，这样就可以实现队列
- 两个队列实现一个栈
    - 思路：根据栈和队列的特点来实现，
    - 开始入栈就入队列A，出栈需要把队列A的所有元素除了最后一个都放入队列B，然后把最后一个元素拿出来，就实现了出栈操作。稍后入栈的话，就把所有的元素都放入队列无为空的队列里。
- string 转 int
    - 思路：比如字符9在ASCII码表里的值为：39，而字符0在ASCII码表里的值为:0，所以用’9’-‘0’就得到了字符’9’对应的真实数字9 
- 合并 k 个已排序的链表
- 给已排序的链表去重
